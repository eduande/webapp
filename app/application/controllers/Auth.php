      ...
      if ($query->num_rows() == 1) {
        ...
        if (password_verify($password, $row->password)) {
          if ($row->status == 'expired') {
            $this->session->set_flashdata('error', 'Su cuenta ha expirado.');
            redirect(base_url('login'));
          } else if ($row->status == 'inactive') {
            $this->session->set_flashdata('error', 'Su cuenta no ha sido activada.');
            redirect(base_url('login'));
          } else {
            if ($rembemr == 'on') {
          ...
          $this->session->set_flashdata('error', 'Tu contraseña es incorrecta.');
          redirect(base_url('login'));
        }
      } else {
        $this->session->set_flashdata('error', 'El nombre de usuario no está registrado.');
        redirect(base_url('login'));
      }
    } else {
