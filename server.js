    ...
    chika.ev.on('connection.update', async (update) => {
        ...
        if (update.qr) {
            ...
            try {
                ...
                io.emit('message', {
                    id: idevice,
                    text: 'Código QR recibido, escanear por favor!'
                });
            } catch {
                io.emit('message', {
                    id: idevice,
                    text: 'QR Error, ¡actualiza la página!'
                });
    ...
    socket.on('logout', async function (data) {
        if (fs.existsSync(`./app_node/session/device-${data.id}.json`)) {
            socket.emit('isdelete', {
                id: data.id,
                text: '<h2 class="text-center text-info mt-4">Cierre de sesión exitoso, permite escanear de nuevo<h2>'
            })
            logoutDEVICE(data.id)
        } else {
            socket.emit('isdelete', {
                id: data.id,
                text: '<h2 class="text-center text-danger mt-4">¡Aún no has iniciado sesión!<h2>'
            })
        }
    })
});
